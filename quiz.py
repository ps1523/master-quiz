...
def showMark(mark):
    sh = Tk()
    sh.title('Your Marks')
    sh.attributes('-fullscreen', True)  # Make the window full screen
    
    st = "Your score is "+str(mark)+"/5"
    rank = getRank(mark)
    rank_st = "Your rank is: " + rank
    mlabel = Label(sh, text=st, fg="black", bg="lightblue", font=("Helvetica", 16))  # Change background color and font size
    mlabel.pack()
    rlabel = Label(sh, text=rank_st, fg="black", bg="lightblue", font=("Helvetica", 16))  # Change background color and font size
    rlabel.pack()
    ...
    b24 = Button(text="Re-attempt", command=myeasy, bg="black", fg="white", font=("Helvetica", 14))  # Increase font size
    b24.pack()
    ...
    b23 = Button(text="Sign Out", command=callsignUpPage, fg="white", bg="black", font=("Helvetica", 14))  # Increase font size
    b23.pack()
...
def loginPage(logdata):
    ...
    login.attributes('-fullscreen', False)  # Make the window full screen
    
    user_name = StringVar()
    password = StringVar()
    
    login_canvas = Canvas(login, width=login.winfo_screenwidth(), height=login.winfo_screenheight(), bg="#B64D4D")
    login_canvas.pack()

    login_frame = Frame(login_canvas, bg="orange")
    login_frame.place(relwidth=0.8, relheight=0.8, relx=0.1, rely=0.1)

    heading = Label(login_frame, text="Quiz App Login", fg="white", bg="orange", font=('calibri 50'))  # Increase font size
    heading.place(relx=0.2, rely=0.1)

    #USER NAME
    ulabel = Label(login_frame, text="Username", fg='white', bg='black', font=('calibri 16'))  # Increase font size
    ulabel.place(relx=0.21, rely=0.4)
    uname = Entry(login_frame, bg='white', fg='black', textvariable=user_name, font=('calibri 16'))  # Increase font size
    uname.config(width=42)
    uname.place(relx=0.31, rely=0.4)

    #PASSWORD
    plabel = Label(login_frame, text="Password", fg='white', bg='black', font=('calibri 16'))  # Increase font size
    plabel.place(relx=0.215, rely=0.5)
    pas = Entry(login_frame, bg='white', fg='black', textvariable=password, show="*", font=('calibri 16'))  # Increase font size
    pas.config(width=42)
    ...
    def check():
        for a, b, c, d in logdata:
            ...
            error = Label(login_frame, text="Wrong Username or Password!", fg='black', bg='white', font=('calibri 16'))  # Increase font size
            error.place(relx=0.37, rely=0.7)
    
    #LOGIN BUTTON
    log = Button(login_frame, text='Login', padx=5, pady=5, width=5, command=check, fg="white", bg="black", font=('calibri 16'))  # Increase font size
...
def signUpPage():
    ...
    sup = Tk()
    sup.title('Quiz App')
    sup.attributes('-fullscreen', False)  # Make the window full screen
    ...
    sup_canvas = Canvas(sup, width=sup.winfo_screenwidth(), height=sup.winfo_screenheight(), bg="#FFBC25")
    sup_canvas.pack()

    sup_frame = Frame(sup_canvas, bg="#BADA55")
    sup_frame.place(relwidth=0.8, relheight=0.8, relx=0.1, rely=0.1)

    heading = Label(sup_frame, text="Quiz App SignUp", fg="#FFA500", bg="#BADA55", font=('calibri 50'))  # Increase font size
    heading.place(relx=0.2, rely=0.1)

    #full name
    flabel = Label(sup_frame, text="Full Name", fg='white', bg='black', font=('calibri 16'))  # Increase font size
    flabel.place(relx=0.21, rely=0.4)
    fname = Entry(sup_frame, bg='white', fg='black', textvariable=fname, font=('calibri 16'))  # Increase font size
    fname.config(width=42)
    fname.place(relx=0.31, rely=0.4)

    #username
    ulabel = Label(sup_frame, text="Username", fg='white', bg='black', font=('calibri 16'))  # Increase font size
    ulabel.place(relx=0.21, rely=0.5)
    user = Entry(sup_frame, bg='white', fg='black', textvariable=uname, font=('calibri 16'))  # Increase font size
    user.config(width=42)
    user.place(relx=0.31, rely=0.5)
    
    #password
    plabel = Label(sup_frame, text="Password", fg='white', bg='black', font=('calibri 16'))  # Increase font size
    plabel.place(relx=0.215, rely=0.6)
    pas = Entry(sup_frame, bg='white', fg='black', textvariable=passW, show="*", font=('calibri 16'))  # Increase font size
    pas.config(width=42)
    pas.place(relx=0.31, rely=0.6)
    
    #country
    clabel = Label(sup_frame, text="Country", fg='white', bg='black', font=('calibri 16'))  # Increase font size
    clabel.place(relx=0.217, rely=0.7)
    c = Entry(sup_frame, bg='white', fg='black', textvariable=country, font=('calibri 16'))  # Increase font size
    c.config(width=42)
    ...
    def addUserToDataBase():
        ...
        if len(fname.get()) == 0 and len(user.get()) == 0 and len(pas.get()) == 0 and len(c.get()) == 0:
            error = Label(text="You haven't enter any field...Please Enter all the fields", fg='black', bg='white', font=('calibri 16'))  # Increase font size
            error.place(relx=0.37, rely=0.7)
            
        elif len(fname.get()) == 0 or len(user.get()) == 0 or len(pas.get()) == 0 or len(c.get()) == 0:
            error = Label(text="Please Enter all the fields", fg='black', bg='white', font=('calibri 16'))  # Increase font size
            error.place(relx=0.37, rely=0.7)
            
        elif len(user.get()) == 0 and len(pas.get()) == 0:
            error = Label(text="Username and password can't be empty", fg='black', bg='white', font=('calibri 16'))  # Increase font size
            error.place(relx=0.37, rely=0.7)

        elif len(user.get()) == 0 and len(pas.get()) != 0:
            error = Label(text="Username can't be empty", fg='black', bg='white', font=('calibri 16'))  # Increase font size
            error.place(relx=0.37, rely=0.7)
    
        elif len(user.get()) != 0 and len(pas.get()) == 0:
            error = Label(text="Password can't be empty", fg='black', bg='white', font=('calibri 16'))  # Increase font size
            error.place(relx=0.37, rely=0.7)
    ...
    #signup BUTTON
    sp = Button(sup_frame, text='SignUp', padx=5, pady=5, width=5, command=addUserToDataBase, bg="black", fg="white", font=('calibri 16'))  # Increase font size
    sp.configure(width=15, height=1, activebackground="#33B5E5", relief=FLAT)
    sp.place(relx=0.4, rely=0.8)

    log = Button(sup_frame, text='Already have a Account?', padx=5, pady=5, width=5, command=gotoLogin, bg="#BADA55", fg="black", font=('calibri 16'))  # Increase font size
...
def menu(abcdefgh):
    ...
    menu_canvas = Canvas(menu, width=menu.winfo_screenwidth(), height=menu.winfo_screenheight(), bg="orange")
    menu_canvas.pack()
...
def easy():
    ...
    easy_canvas = Canvas(e,width=e.winfo_screenwidth(),height=e.winfo_screenheight(),bg="orange")
    easy_canvas.pack()
...
def medium():
    ...
    med_canvas = Canvas(m,width=m.winfo_screenwidth(),height=m.winfo_screenheight(),bg="#101357")
    med_canvas.pack()
    ...
    a.place(relx=0.5, rely=0.48, anchor=CENTER)
...
def difficult():
    ...
    hard_canvas = Canvas(h,width=h.winfo_screenwidth(),height=h.winfo_screenheight(),bg="#101357")
    hard_canvas.pack()
...
def start():
    ...
    root.attributes('-fullscreen', False)  # Make the window full screen
    canvas = Canvas(root, width=root.winfo_screenwidth(), height=930, bg='yellow')
    canvas.grid(column=0, row=1)
    img = PhotoImage(file="img/output-onlinepngtools.png")
    canvas.create_image(600, 250, image=img, anchor=NW)

    button = Button(root, text='Start', command=signUpPage, bg="red", fg="yellow", font=('calibri 16'))  # Increase font size
    button.configure(width=158, height=2, activebackground="#33B5E5", relief=RAISED)
